...
type ButtonLinkProps = {
  ...
  icon?: LucideIcon;
  variant?: "primary" | "secondary" | "light" | "outlineLight" | "gold";
  className?: string;
};

export function ButtonLink({ href, children, icon: Icon, variant = "primary", className = "" }: ButtonLinkProps) {
  const base =
    "focus-ring inline-flex min-h-12 w-full items-center justify-center gap-2 border px-5 text-center text-sm font-extrabold leading-tight shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl active:translate-y-0 sm:w-auto";
  const variants = {
    ...
    gold: "border-[#d7aa52] bg-[#d7aa52] text-[#071f3b] hover:bg-[#f0c66c]",
  };
